...
treeFUN <- function(df,site_i,pub_year_i,study_start_i,
                    study_end_i,no_pat_i,poc_i,source_i,country_i,type) {
  ...
  if (type=="plot") {
    ...
    pre_table1 <- load %>% 
      dplyr::select(cov_id,cite,title,journal,cancer_type,
    ...
    reactable(table[1:5],
              ...
              details=function(index) {
                nest_table <- table %>%
                  slice(index) %>% 
                  dplyr::select(-c(cite,title,journal,inequality_type,
                                   cancer_type))
...
mapFUN <- function(df,df_geo,site_i,pub_year_i,study_start_i,
                   study_end_i,no_pat_i,poc_i,source_i,country_i,map_type) {
